...
class Persona extends Controller
{
    ...
    public function index()
    {
        ...
        $data1 = '';
        $this->views->getView('Administracion', "Persona", $data, $data1);
    }
    ...
    public function registrar()
    {
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if (empty($tipo_persona)) {
                $error_msg .= 'El campo <b>tipo persona</b> es obligatori111o.<br>';
            }
            if (empty($nombre)) {
                $error_msg .= 'El campo <b>nombre</b> es obligatori111o.<br>';
            }
            if (strlen($nombre) < 5 || strlen($nombre) > 50) {
                $error_msg .= 'El Persona debe tener entre 3 y 50 caracteres. <br>';
            }
            if (empty($direccion)) {
            ...
            if (empty($id)) {
                $data = $this->model->registrar($tipo_persona, $dni, $ruc, $nombre, $direccion, $ubigeo, $email, $celular);
                if ($data > 0) {
                    echo json_encode(['msg' => 'Persona registrada', 'icono' => 'success']);
                } else {
                    echo json_encode(['msg' => 'Error al registrar', 'icono' => 'error']);
                }
            } else {
                $data = $this->model->modificar($tipo_persona, $dni, $ruc, $nombre, $direccion, $ubigeo, $email, $celular, $estado, $id);
                if ($data == 1) {
                    echo json_encode(['msg' => 'Persona modificada', 'icono' => 'success']);
                } else {
